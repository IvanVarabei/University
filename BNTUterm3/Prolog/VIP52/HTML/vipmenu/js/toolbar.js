...
//		"<DIV ID='idMSMenuPane'  STYLE='position:absolute;top:0;left:250;height:0;width:10;background-color:black;float:right;' NOWRAP><!--MS_MENU_TITLES--></DIV>" + 
//		"</DIV>" +
//		"<DIV ID='idRow2' STYLE='position:relative;left:250;height:0;'>" +
//		"</DIV>" +
//		"<DIV ID='idRow3' STYLE='position:relative;height:0;width:100%'>" +
		"<DIV ID='idICPMenuPane' STYLE='position:absolute;top:2;left:190;height:1.5em;background-color:black; z-index=1' NOWRAP><!--ICP_MENU_TITLES--></DIV>" +
		"</DIV>" +
//		"</DIV>" +
...
//	idICPMenuPane.style.width = ToolbarMinWidth + w;
	idICPMenuPane.style.width = document.body.clientWidth;
}
...
function setSubMenuWidth(MenuIDStr, WidthType, WidthUnit)
{
	...
	{
		arrMenuInfo[i].type = "A";
	  	arrMenuInfo[i].unit = ICPSubMenuWidth;
	}
	else
	{
...
function addMenu(MenuIDStr, MenuDisplayStr, MenuHelpStr, MenuURLStr, TargetStr, bICPMenu)
{
	...
	if (MenuURLStr != "")
	{
		if (bICPMenu)
			...
			MenuStr += " HREF='" + formatURL(MenuURLStr, ("PDC_" + MenuDisplayStr)) + "'";
	}
	else
...
//	x2 = x + window.event.srcElement.offsetWidth;
	x2 = x + ICPSubMenuWidth;
//	y  = (IsMSMenu)? //Leo
//		 (idRow1.offsetHeight) :
//	 (idRow1.offsetHeight + idRow2.offsetHeight + idRow3.offsetHeight + 37);
    y = idICPMenuPane.offsetTop + idICPMenuPane.clientHeight;
...
function hideMenu()
{
	if (ToolbarMenu != null && ToolbarMenu != StartMenu) 
	{
		...
		if ( (event.clientX >= (x+5) && event.clientX <= x2) &&
		 ((IsMenuDropDown == true  && cY > (y-12) && cY <= y2)      ||
...
function menuSetPos(x,y){
	idICPMenuPane.style.left=x;
	idICPMenuPane.style.top=y; 
}